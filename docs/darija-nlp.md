...
Defined in `app/preprocessing/darija_tokenizer.py`:
...
Sticky facts can include a `language_override` field (`en` | `fr` | `ar`) that forces the response language regardless of input script detection. This allows users to specify their preferred language explicitly.
